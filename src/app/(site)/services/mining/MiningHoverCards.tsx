// components/mining/MiningHoverCards.tsx
"use client";
...
function MiningHoverCard({ item, index }: { item: Item; index: number }) {
...
export default function MiningHoverCards({ items }: { items: Item[] }) {
  return (
    <div className="px-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {items.map((item, i) => (
        <MiningHoverCard key={item.title} item={item} index={i} />
      ))}
    </div>
  );
}